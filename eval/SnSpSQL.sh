...
-- \COPY KNOWN_CNVS FROM '${workDir}/../../gpc_wave2_batch1/gs_dels_flt.genotypes.txt' DELIMITER E'\t' CSV HEADER;
 \COPY KNOWN_CNVS FROM '${workDir}/../../gpc_wave2_batch1/gs_dels_best.genotypes.txt' DELIMITER E'\t' CSV HEADER;
INSERT INTO ALL_KNOWN_CNVS SELECT *, 'GS_DELS', 'SAMPLESEG' FROM KNOWN_CNVS;
...
SELECT k.SAMPLE AS k_SAMPLE, k.SOURCE as k_SOURCE, p.SAMPLE AS p_SAMPLE, p.SOURCE as p_SOURCE,
       ...
       p.END_POS
  INTO OVERLAP
  FROM (SELECT * FROM ALL_KNOWN_CNVS WHERE SOURCE='GS_DELS') AS k FULL OUTER JOIN (SELECT * FROM ALL_PRED_CNVS WHERE START_POS < END_POS AND source='BAYES') AS p ON 
